...
export async function getWatchPartySession(roomId) {
  try {
    const { rows } = await sql`
      SELECT * FROM watchparty_sessions
...
export async function startWatchPartySession(session) {
  try {
    await sql`UPDATE watchparty_sessions SET active = false, ended_at = NOW() WHERE room_id = ${session.roomId}`
    await sql`
      INSERT INTO watchparty_sessions (id, room_id, host_user_id, host_pseudo, active, started_at)
      VALUES (${session.id}, ${session.roomId}, ${session.hostUserId}, ${session.hostPseudo}, true, NOW())
    `
    return true
  } catch (err) {
    if (err.code === '42P01') return false
...
export async function endWatchPartySession(roomId, userId) {
  try {
    await sql`
      UPDATE watchparty_sessions
      SET active = false, ended_at = NOW()
      WHERE room_id = ${roomId} AND host_user_id = ${userId} AND active = true
    `
    return true
  } catch (err) {
    if (err.code === '42P01') return false
...
export async function getWatchPartyPeers(sessionId) {
  try {
    const { rows } = await sql`
      SELECT * FROM watchparty_peers
...
export async function getWatchPartyPeer(peerId) {
  try {
    const { rows } = await sql`SELECT * FROM watchparty_peers WHERE id = ${peerId} LIMIT 1`
    return rows[0] || null
  } catch (err) {
    if (err.code === '42P01') return null
    throw err
...
export async function createWatchPartyPeer(peer) {
  try {
    await sql`
      INSERT INTO watchparty_peers (id, room_id, session_id, viewer_user_id, viewer_pseudo, offer, viewer_candidates, host_candidates, created_at, updated_at)
      VALUES (${peer.id}, ${peer.roomId}, ${peer.sessionId}, ${peer.viewerUserId}, ${peer.viewerPseudo}, ${peer.offer}, '[]', '[]', NOW(), NOW())
    `
    return true
  } catch (err) {
    if (err.code === '42P01') return false
...
export async function addWatchPartyCandidate(peerId, side, candidate) {
  try {
    const peer = await getWatchPartyPeer(peerId)
    if (!peer) return false
    const column = side === 'host' ? 'host_candidates' : 'viewer_candidates'
    const current = JSON.parse(peer[column] || '[]')
    if (!current.includes(candidate)) current.push(candidate)

    if (side === 'host') {
      await sql`
        UPDATE watchparty_peers
        SET host_candidates = ${JSON.stringify(current)}, updated_at = NOW()
        WHERE id = ${peerId}
      `
    } else {
      await sql`
        UPDATE watchparty_peers
        SET viewer_candidates = ${JSON.stringify(current)}, updated_at = NOW()
        WHERE id = ${peerId}
      `
    }
    return true
...
export async function answerWatchPartyPeer(peerId, answer) {
  try {
    await sql`
      UPDATE watchparty_peers
