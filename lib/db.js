...
export async function getCustomAvatars() {
  try {
    const { rows } = await sql`
      SELECT id, pseudo, avatar_emoji, avatar_hue, avatar_url, tag_label, tag_color, moderator
      FROM users
...
/** Tag affiché à côté du pseudo (attribué par l'admin du site). */
export async function updateUserTag(userId, label, color) {
  try {
    await sql`UPDATE users SET tag_label = ${label}, tag_color = ${color} WHERE id = ${userId}`
  } catch (err) {
    if (err.code !== '42703') throw err
