...
/** Choisit la meilleure bande-annonce YouTube (trailer officiel, sinon le premier trailer, sinon un teaser). */
function pickTrailerKey(videos) {
  const list = (videos?.results || []).filter(v => v.site === 'YouTube')
  const trailer = list.find(v => v.type === 'Trailer' && v.official) || list.find(v => v.type === 'Trailer')
  const teaser = list.find(v => v.type === 'Teaser')
...
export async function getTmdbDetails(mediaType, tmdbId) {
  // include_video_language : la plupart des bandes-annonces ne sont taguées
  // qu'en anglais sur TMDB, on les inclut en plus du français.
  const data = await tmdbFetch(`/${mediaType}/${tmdbId}`, { append_to_response: 'credits,videos', include_video_language: 'fr,en' })
